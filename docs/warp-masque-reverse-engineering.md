...
## 5. 官方前向代理的约束：plain CONNECT 只能用 H3

官方的逐目标 forward-proxy 实现不能把 plain CONNECT 搬到 H2，有三条独立证据：
...
**结论**：即使官方客户端默认 `h3_with_h2_fallback`，它的逐目标代理模式仍是 H3-only。但这不等于 SOCKS 前端不能使用 H2：`warp-go` 的 H2 后端先建立 Connect-IP，再把无特权用户态 netstack 的 TCP/UDP socket 暴露给 SOCKS。它没有尝试在 H2 上发送 plain CONNECT，也不需要内核 TUN、root 或路由修改。

### 5.1 H2 隧道细节 **[二进制 + 运行验证]**
...
- 实际握手参数：TCP `162.159.198.2:443`，SNI `consumer-masque.cloudflareclient.com`，CONNECT URI `https://cloudflareaccess.com`，请求头 `cf-connect-proto: cf-connect-ip` 与 `pq-enabled: false`
- 边缘可能不返回 `h2` ALPN，但接受连接后显式发送的 HTTP/2 connection preface；因此不能依赖普通 `net/http` 的 ALPN 自动升级，需在 TLS socket 上直接创建 `http2.ClientConn`
...
纯代理客户端：**免 root、无内核 TUN、无路由改动**。有两个可选后端：

```
                    ┌─► plain CONNECT ─► QUIC/H3 ───────────────┐
SOCKS5 客户端 ──────┤                                           ├─► WARP 边缘 ─► 目标
                    └─► gVisor netstack ─► Connect-IP/TCP/H2 ───┘
                                      ▲
...
| 文件 | 职责 |
|---|---|
| `main.go` | 参数解析、后端选择、注册编排、SOCKS5 监听与数据面自测 |
| `registration/registration.go` | 两步注册、状态持久化、边缘公钥固定回调 |
| `tunnel/masque.go` | QUIC/H3 连接管理、SOCKS5 TCP、DoH 解析 |
| `tunnel/udp.go` | H3 后端的本机直连 SOCKS5 UDP ASSOCIATE |
| `tunnel/h2.go` | TCP/TLS/H2 Connect-IP、用户态 netstack、PING 与自动重连 |
| `tunnel/h2_socks.go` | H2 后端的 SOCKS5 TCP 与认证 |
| `tunnel/h2_udp.go` | H2 后端的双栈 WARP UDP ASSOCIATE |
...
-transport  auto / h3 / h2（默认 auto；Railway 使用 h2）
-self-test  启动后通过自己的 SOCKS 访问 cdn-cgi/trace 并验证 warp=on
-reg    注册并退出（已有注册则跳过）
-del    注销并删除本地注册
```
...
#### H2 Connect-IP backend

`H2Client` 把注册分配的 IPv4/IPv6 地址交给 WireGuard 的 gVisor netstack。这个 TUN device 完全在进程内存中：netstack 发出的原始 IP 包由单一出站 pump 写入 capsule type 0，边缘返回的 capsule 由入站 pump 注回 netstack。SOCKS TCP 使用 `tunNet.DialContext`，SOCKS UDP 使用双栈 `tunNet.ListenUDP`，域名解析也由该 stack 向隧道内 DNS 发包。

每条 H2 连接是一个 `h2Bundle`，包含 TCP/TLS socket、显式 `http2.ClientConn`、Connect-IP capsule stream 和生命周期 context。永久 TUN reader 与连接生命周期分离；重连只原子替换 current bundle，不重建 netstack。这样本地 socket 地址和已分配隧道地址保持稳定。

入站读取、出站写入或 30 秒一次的 HTTP/2 PING 任一失败都会清除 current bundle，关闭旧连接并在 1 秒后重拨。重连期间 netstack 的首个待发包会等待新 bundle，后续 TCP 重传负责恢复新流量。边缘会关闭长时间无 HTTP/2 帧的空闲连接，因此 PING 不是可选优化。

`auto` 给 H3 初始连接总计 10 秒预算；QUIC 黑洞时随后连接 H2。Railway 应显式指定 `h2`，避免无意义的 10 秒启动延迟。
...
H3 后端对齐 §8.3 的官方设计：**单条 HTTP/2 连接承载所有查询，每查询一条 H2 流**。该连接建立在一条 H3 CONNECT 流内部：
...
H2 后端不使用上述嵌套 DoH。`netstack.Net.DialContext` 与 UDP 目标解析调用 netstack 自带 resolver，DNS 请求和响应与其他 IP 包一起经过 Connect-IP；宿主解析器只用于启动前解析显式 `-ip hostname:port`。
...
H3 后端受 plain CONNECT 限制，UDP 直接从宿主网络栈发出：**TCP 走隧道、UDP 走本地**。H2 后端则通过 Connect-IP 的用户态 netstack 创建 IPv4/IPv6 UDP socket，数据报和域名解析均经过 WARP。当前实现始终提供 UDP ASSOCIATE，没有关闭开关。

实现要点：首个数据报到达时**钉住客户端源地址**以防离路径劫持；拒绝 `FRAG != 0`；60 秒滚动空闲超时，并在 TCP 控制连接关闭时拆除。H2 为两个隧道地址族各维护一个 remote socket，按解析结果选择，回包统一封装为 RFC 1928 UDP frame。
...
| 代理 CONNECT | H3 only | H3 后端一致 | 见 §5 |
| Connect-IP over H2 | 内核 TUN 数据面 | 用户态 netstack + SOCKS | 协议一致，前端不同 |
...
| H2 隧道回退 | 有（仅 Connect-IP） | 有（`auto`） | 本实现顺序尝试，不竞速 |
| TUN / Connect-IP | 内核接口 | 进程内 netstack | 本实现免 root |
...
1. **H3 仍依赖出站 UDP**。被封锁的平台必须用 `-transport h2` 或 `auto`；Railway 部署脚本固定使用 H2。
2. **两种重连语义不同**。H3 空闲断线后由下一请求惰性重连；H2 用 PING 检测并后台重连。两者断线瞬间的既有 TCP 会话都可能中断，客户端仍需重试。
3. **H3 的 UDP 不走隧道**（§9.6）。严格避免源地址泄漏时应使用 H2，或在上层禁止 H3 客户端发 UDP。
4. **PQ 密钥交换无法对齐**（§9.5）。
5. **无 DNS 健康跟踪与故障转移**。上游列表按序尝试，失败仅按传输错误分类处理。
6. **H3 隧道内没有 Happy Eyeballs**。A 与 AAAA 并发查询，但只要有 A 记录就一律用 IPv4，不做连通性探测；H2 netstack 使用自己的地址选择逻辑。注意这与 `-ip` 无关——`-ip` 决定的是如何到达边缘。
